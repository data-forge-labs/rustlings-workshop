...
| 15 | **ClosureTraits** — closure traits & custom iterators | `Fn`/`FnMut`/`FnOnce`, `move` closures, custom `Iterator` impl, `IntoIterator` | Project |
...
5. Finish with **12-RustIterators** for iteration, then **15-ClosureTraits** for closure traits and custom iterators
6. Practice mutation with **13-MutableFruitSalad**
7. Cap the section with **14-ProfileBenchmark** — measure what you've built with `criterion`, configure Cargo profiles, and observe the 10-50x dev-vs-release gap
