...
use russian_troll_tweets::{
    build_retweet_graph, can_reach, find_bridge_users, influence_score, top_users, tweet_count_by_user, Tweet,
};

fn main() {
    let tweets = vec![
        Tweet { user: "ira_troll_1".into(), content: "Dividing the nation!".into(), retweet_of: None },
        Tweet { user: "user_a".into(), content: "RT @ira_troll_1".into(), retweet_of: Some("ira_troll_1".into()) },
        Tweet { user: "user_b".into(), content: "RT @ira_troll_1".into(), retweet_of: Some("ira_troll_1".into()) },
        Tweet { user: "ira_troll_2".into(), content: "Election rigged!".into(), retweet_of: None },
        Tweet { user: "user_c".into(), content: "RT @ira_troll_2".into(), retweet_of: Some("ira_troll_2".into()) },
    ];

    let graph = build_retweet_graph(&tweets);
    let counts = tweet_count_by_user(&tweets);
    let top = top_users(&counts, 3);

    println!("=== Russian Troll Tweet Analysis ===");
    println!("\nRetweet graph edges:");
    ...
    for (user, count) in &top {
        println!("  {}: {} tweets", user, count);
    }

    println!("\nInfluence scores:");
    println!("  ira_troll_1: {}", influence_score(&graph, "ira_troll_1"));
    println!("  ira_troll_2: {}", influence_score(&graph, "ira_troll_2"));

    println!("\nReachability:");
    println!("  user_a -> ira_troll_1: {}", can_reach(&graph, "user_a", "ira_troll_1"));
    println!("  user_c -> ira_troll_1: {}", can_reach(&graph, "user_c", "ira_troll_1"));

    println!("\nBridge users (appearing in multiple components): {:?}", find_bridge_users(&graph));
}
