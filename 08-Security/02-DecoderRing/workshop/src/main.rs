...
use clap::Parser;
use decoder_ring::{gen_counts, guess_shift_parallel};
...
struct Args {
    ...
    /// Show English letter frequency statistics
    #[arg(short, long)]
    stats: bool,

    /// Guess the shift used in the Caesar cipher
    #[arg(short, long)]
    guess: bool,
}
...
fn main() {
    ...
    if args.stats {
        ...
        println!("English letter frequencies:");
    ...
    if args.guess {
        let (best_shift, decrypted, score) = guess_shift_parallel(&args.message, 26);
        println!(
            "Best shift: {} (out of 26), score: {}",
            best_shift, score
        );
