...
fn main() {
    ...
    println!("--- simple_hash ---");
    println!("hash of \"hello\": {}", rust_crypto_hashes::simple_hash("hello"));
    println!("hash of \"world\": {}", rust_crypto_hashes::simple_hash("world"));

    println!("\n--- xor_checksum ---");
    println!("checksum of [1,2,3,4]: {}", rust_crypto_hashes::xor_checksum(&[1, 2, 3, 4]));

    println!("\n--- is_deterministic ---");
    println!("\"hello\" is deterministic: {}", rust_crypto_hashes::is_deterministic("hello"));

    println!("\n--- avalanche_effect ---");
    println!("\"hello\" change at 2: {}", rust_crypto_hashes::avalanche_effect("hello", 2));

    println!("\n--- hash_algorithms ---");
    for algo in rust_crypto_hashes::hash_algorithms() {
        println!("  - {algo}");
    }

    println!("\n--- hash_properties ---");
    for prop in rust_crypto_hashes::hash_properties() {
        println!("  - {prop}");
    }
}
