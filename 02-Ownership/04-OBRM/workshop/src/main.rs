...
use obrm::{raii_demo, ownership_transfer, borrow_resource, Resource, obrm_concepts};
...
fn main() {
    ...
    println!("Ownership transfer count: {count}");

    let res = Resource::new(42);
    let id = borrow_resource(&res);
    println!("Borrowed resource id: {id}");
    println!("Resource still open: {}", res.is_open());

    println!("\nOBRM Concepts:");
    for concept in obrm_concepts() {
        println!("  - {concept}");
    }
}
