...
driver = GraphDatabase.driver("bolt://localhost:7687")
with driver.session() as session:
    session.run("CALL gds.degree.stream('myGraph') ...")  # network call per query
```

**Rust fix:** Implement all three centrality algorithms in a single `Graph` struct, no database required — self-contained, offline, and ~100x faster than a network round-trip:

```rust
pub struct Graph { pub adjacency: HashMap<usize, Vec<usize>> }
impl Graph {
