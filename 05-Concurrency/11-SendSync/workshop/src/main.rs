...
use send_sync::{create_thread_safe_wrapper, demonstrate_mutex_send_sync, verify_send, verify_send_sync, verify_sync, Wrapper};

fn main() {
    let i = verify_send(42i32);
    println!("verify_send(42) = {i}");

    let i = verify_sync(99i32);
    println!("verify_sync(99) = {i}");

    let i = verify_send_sync(100i32);
    println!("verify_send_sync(100) = {i}");

    let w = create_thread_safe_wrapper(7);
    println!("create_thread_safe_wrapper(7) = Wrapper({})", w.0);

    let result = demonstrate_mutex_send_sync();
    println!("demonstrate_mutex_send_sync() = {result}");
}
