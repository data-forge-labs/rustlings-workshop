...
//!
use rayon_challenge::{compute_speedup, cpu_count, parallel_filter, parallel_frequency, parallel_increment, parallel_sum};
...
fn main() {
    ...
    println!("Sum: {}", parallel_sum(data.clone()));
    println!("Incremented: {:?}", parallel_increment(data.clone()));
    println!("Filtered (>5): {:?}", parallel_filter(data.clone(), 5));
    println!("CPU count: {}", cpu_count());

    let words = vec!["rayon", "is", "fast", "rayon", "rocks"];
    println!("Frequencies: {:?}", parallel_frequency(words));

    println!("Speedup (1M items): {:.2}x", compute_speedup(1_000_000));
}
