...
use atomics::{
    acquire_release_demo, atomic_counter, atomic_flag_toggle, fetch_add_demo,
    relaxed_ordering_demo,
};

fn main() {
    let count = atomic_counter(100);
    println!("atomic_counter(100) = {count}");

    let flag = atomic_flag_toggle();
    println!("atomic_flag_toggle() = {flag}");

    let (a, b) = relaxed_ordering_demo();
    println!("relaxed_ordering_demo() = ({a}, {b})");

    let (val, flg) = acquire_release_demo();
    println!("acquire_release_demo() = ({val}, {flg})");

    let sum = fetch_add_demo(vec![10, 20, 30, 40, 50]);
    println!("fetch_add_demo(vec![10,20,30,40,50]) = {sum}");
}
