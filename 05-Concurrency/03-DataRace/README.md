...
# Run with lock: counter = 1000000 (10 threads x 100000)
# Run without lock: counter approx 300000 (wrong!)
```
...
Open `workshop/src/lib.rs`. Each function has a `todo!()` stub. Replace them as you progress:

### Functions to implement

| Function | Concept | Tests |
|---|---|---|
| `greet_thread()` | thread::spawn + join | step_01_threads (2 tests) |
| `sum_in_parallel()` | Split work across threads | step_01_threads (3 tests) |
| `move_closure_example()` | Move closures | step_02_move_closures (3 tests) |
| `shared_counter_arc_mutex()` | Arc\<Mutex\> | step_03_arc_mutex (3 tests) |
| `shared_counter_rwlock()` | RwLock | step_04_rwlock (3 tests) |
| `condvar_coordinate()` | Condition variable | step_05_condvar (3 tests) |

Run `cd workshop && cargo test` after implementing each function.
