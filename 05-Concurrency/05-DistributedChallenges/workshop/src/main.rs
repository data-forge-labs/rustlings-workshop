...
use challenges_opportunities_distributed::*;

fn main() {
    println!("--- CAP Theorem Validator ---");
    println!("C=true, A=false, P=true => {}", validate_cap_pair(true, false, true));
    println!("C=false, A=true, P=true => {}", validate_cap_pair(false, true, true));
    println!("C=true, A=true, P=false => {}", validate_cap_pair(true, true, false));
    println!("C=true, A=true, P=true => {}", validate_cap_pair(true, true, true));
    ...
    println!("\n--- Leader Election ---");
    let nodes = 5;
    println!("Leader elected from {} nodes: Node {}", nodes, simulate_leader_election(nodes));

    println!("\n--- Quorum Read ---");
    let writes = vec![1, 2, 2, 3, 2];
    let quorum = 3;
    match simulate_quorum_read(writes, quorum) {
        Some(val) => println!("Value {} reaches quorum of {}", val, quorum),
        None => println!("No value reaches quorum of {}", quorum),
    }
}
