...
use data_races_race_conditions::{
    cell_counter, cell_string, refcell_borrow_error, refcell_demo, simulate_race_condition,
};

fn main() {
    println!("=== Cell/RefCell Demo ===\n");

    let count = cell_counter(10);
    println!("cell_counter(10) = {count}");

    let s = cell_string("Rust", " Data Engineering");
    println!("cell_string(\"Rust\", \" Data Engineering\") = \"{s}\"");

    let doubled = refcell_demo(vec![1, 2, 3, 4, 5]);
    println!("refcell_demo([1,2,3,4,5]) = {doubled:?}");

    match refcell_borrow_error() {
        Ok(msg) => println!("refcell_borrow_error() = Ok(\"{msg}\")"),
        Err(e) => println!("refcell_borrow_error() = Err(\"{e}\")"),
    }

    let race = simulate_race_condition();
    println!(
        "simulate_race_condition() = {race} (expected 8000 without race)"
    );
}
